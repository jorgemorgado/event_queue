#!/bin/bash
#
# The host notification command is called by the poller and puts
...
# to push host notifications to the EQ (just make sure to respect the order
# of the arguments).
#
...
/usr/bin/curl \
  ...
  -X PUT \
  -d "{${CONTENT}}" \
  http://eqhost:5555/eq/new-host-event
